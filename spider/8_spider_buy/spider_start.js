...
//查找配置文件
var result = send.post(config.db_server_url + config.db_list_detail_cleaned_query,{ conf: { dealStatus: 2, web_type: "buy" } });
...
for (let i = 0, len = data.length; i < len; i++) {
	...
	try {
		...
		send.post(config.db_server_url + config.db_buy_add, addparam);	//向目标接口新增记录
		send.post(config.db_server_url + config.db_list_detail_cleaned_edit, param);	//修改状态
	} catch (e) {
