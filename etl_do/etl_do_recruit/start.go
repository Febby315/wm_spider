...
func sysDataTimer(appflagmap map[string]interface{}) {
	...
	fmt.Println("定时间隔秒", time.Duration(_intervaltime)*time.Second, appflagmap["recruitConf"])
	for {
		select {
		case <-_timer.C:
			if recruitConf := appflagmap["recruitConf"]; recruitConf != nil {
				server.SynRecruitData(recruitConf.(string))
			}
		}
	}
}
...
func main() {
	...
	fmt.Println("paramConf", paramConf, "appflag", appflag)
	//获取查询条件
