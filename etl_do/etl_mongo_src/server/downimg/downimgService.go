...
func List(w http.ResponseWriter, r *http.Request, p httprouter.Params) {
	...
	if qparam.Conf["dealStatus"] != nil { //有标记按照条件查询
		if strings.ToLower(qparam.Conf["dealStatus"].(string)) != "_all" {
			_param["dealStatus"], _ = strconv.Atoi(qparam.Conf["dealStatus"].(string))
		}
	}
	log.Println("查询参数", _param)
	count, resultdata := 0, []Downimg{}
