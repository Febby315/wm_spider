...
func SynPriceData(conf string) {
	...
	qpriceparam["conf"] = confmap

	//获取数据tagArticle
	result := utils.HTTPPost(utils.GetStringValue("from_interface", "db_src_server_url")+"/price/list", qpriceparam) //根据起始位置查询数据
	...
	if err == nil {
		cols := strings.Split(utils.GetStringValue("sour_need_del_col", "del_price_col"), ",")
		_id := "0"
		for _, value := range resultarray.Data {
			...
			utils.HTTPPost(utils.GetStringValue("to_interface", "db_tar_price_url"), value)
			fmt.Println(_id)
		}
		if _id != "0" {
