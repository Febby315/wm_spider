...
func sysDataTimer(appflagmap map[string]interface{}) {
	...
	for {
		select {
		case <-_timer.C:
			if updownloadConf := appflagmap["updownloadConf"]; updownloadConf != nil {
				fmt.Println("updownloadConf", updownloadConf)
				server.DownLoad(updownloadConf.(string))
			}
...
func main() {
	...
	fmt.Println("etl_do_updownload", paramConf)
	//用户的权限
